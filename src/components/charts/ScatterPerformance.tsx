...
export const ScatterPerformance: React.FC<ScatterPerformanceProps> = ({ data, height = 300 }) => {
  ...
  const calculateRegression = React.useMemo(() => {
    ...
    const n = data.length;
    const sumX = data.reduce((sum, point) => sum + point.thermalLoad, 0);
    const sumY = data.reduce((sum, point) => sum + point.powerKw, 0);
    const sumXY = data.reduce((sum, point) => sum + point.thermalLoad * point.powerKw, 0);
    const sumXX = data.reduce((sum, point) => sum + point.thermalLoad * point.thermalLoad, 0);
